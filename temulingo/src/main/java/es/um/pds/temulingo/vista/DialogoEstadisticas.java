...
public class DialogoEstadisticas extends JDialog {
	...
	private JPanel crearPanelConTitulo(String titulo) {
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		panel.setBorder(BorderFactory.createCompoundBorder(new TitledBorder(titulo),
	...
	private JPanel crearPanelChart() {
		...
		panelConChart.setBorder(BorderFactory.createCompoundBorder(new TitledBorder(TITULO_GRAFICO),
	...
	private void personalizarGrafico(JFreeChart chart) {
		...
		plot.setLabelGenerator(new org.jfree.chart.labels.StandardPieSectionLabelGenerator("{0}: {2}", FORMATO_ENTERO,
				FORMATO_PORCENTAJE));
