...
public class PanelPreguntaDinamico extends JPanel {
	...
	// Constantes de diseño (duplicadas para mantener la lógica de UI independiente,
	// aunque podríamos centralizarlas más)
	...
	// Componentes específicos para cada tipo de pregunta (para poder acceder a
	// ellos)
	...
	public PanelPreguntaDinamico() {
		...
		panelContenidoEspecifico = new JPanel();
	...
	public void establecerPregunta(Pregunta pregunta) {
		...
		grupoOpcionesTest = null;
		campoRespuestaTexto = null;
		...
		if (campoRespuestaTexto != null && listenerEnvioGeneral != null) {
			for (ActionListener al : campoRespuestaTexto.getActionListeners()) {
				campoRespuestaTexto.removeActionListener(al);
			}
		}
		botonAccion = null;
		...
		if (pregunta instanceof PreguntaTest) {
			...
			if (listenerEnvioGeneral != null) {
				botonAccion.addActionListener(e -> {
					listenerEnvioGeneral.actionPerformed(new java.awt.event.ActionEvent(this,
							java.awt.event.ActionEvent.ACTION_PERFORMED, "SALTAR_ERROR"));
				});
			}
		}
		if (botonAccion != null) {
	...
	private void renderizarPreguntaHuecos(PreguntaHuecos pregunta) {
		...
		campoRespuestaTexto.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(COLOR_PRIMARIO, 2), new EmptyBorder(12, 15, 12, 15)));
		...
		if (listenerEnvioGeneral != null) {
			campoRespuestaTexto.addActionListener(listenerEnvioGeneral); // Para el evento ENTER
			botonAccion.addActionListener(listenerEnvioGeneral); // Para el clic del botón
		}
